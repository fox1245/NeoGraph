<!-- neograph-i18n: source=bindings/python/examples/README.md locale=zh-CN source_sha256=1c9ad12b9098111ceefe6e550a72390df5e35292924c7ad4bd899bac79e9519f -->
# Python API 示例
...
| 26 | [`26_mcp_tools.py`](26_mcp_tools.py) | offline | MCP (#95)：`ng.mcp.MCPClient(url).get_tools()` 拉取 remote tool catalogue，并直接交给 `NodeContext`。它会启动自己的 MCP server，因此无网络也能运行。测量真正重要的东西 — 三个 0.4 s MCP calls 经 HTTP 在 0.41 s 内完成 — 并明确说明哪里不成立：stdio 只有一根 pipe，因此这些 calls 会串行。 |
