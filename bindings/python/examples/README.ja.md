<!-- neograph-i18n: source=bindings/python/examples/README.md locale=ja source_sha256=9e936b9adcdabe02b5173ddcdbea7246c3329915e68958acd7c3726c8e1ad55e -->
# Python API の例
...
| 26 | [`26_mcp_tools.py`](26_mcp_tools.py) |オフライン | MCP (#95): `ng.mcp.MCPClient(url).get_tools()` はリモート ツールカタログを取得し、直接 `NodeContext` に渡します。独自の MCP サーバーを起動するため、ネットワークなしで実行されます。同名の繰り返し呼び出しは既定で直列です。スレッド対応デモは `ToolExecutionPolicyRegistry` で `fetch` だけを明示的に Reentrant と宣言してから、HTTP の 0.4 秒呼び出し 3 回が 0.41 秒で完了することを測定します。stdio も JSON-RPC ID を多重化しますが、重複にはそのホストポリシーと並行サーバーの両方が必要です。 |
