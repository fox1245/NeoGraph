<!-- neograph-i18n: source=bindings/python/examples/README.md locale=ko source_sha256=f83696b352140f2c77392207424b16e3d297e7b183c67f5d1fd26347c1d7e911 -->
# Python API 예제
...
| 16 | [`16_deep_research_chat.py`](16_deep_research_chat.py) | **Responses(WS/HTTP)** | `조사해줘 / research / investigate`에서 3-way 병렬 딥 리서치 서브그래프로 전환하는 멀티턴 Gradio 채팅입니다. 공식 OpenAI는 기본적으로 WebSocket을 사용하고, 호환 게이트웨이는 빌드에 포함된 HTTP/2 또는 HTTP/1.1을 자동 선택합니다. `NG_RESPONSES_TRANSPORT`로 재정의할 수 있습니다. `pip install gradio`가 필요합니다. |
| 17 | [`17_deep_research_crawl4ai.py`](17_deep_research_crawl4ai.py) | **Responses + Crawl4AI + Postgres** | 16번과 동일한 전송 방식 인식 채팅 형태이지만, 연구자들은 로컬 Crawl4AI 컨테이너(`docker run unclecode/crawl4ai`)를 통해 실제로 웹을 검색하며 상태는 Postgres(`PostgresCheckpointStore`)에 영속화됩니다. 둘 다 환경 변수로 선택할 수 있고, 없으면 정상적으로 폴백합니다. Postgres 경로는 `-DNEOGRAPH_BUILD_POSTGRES=ON`으로 소스 빌드합니다. |
| 18 | [`18_node_cache.py`](18_node_cache.py) | **OpenAI** | `engine.set_node_cache_enabled("ask", True, CacheScope.Reusable)` — 이후 동일한 입력의 실행은 캐시된 `NodeResult`를 0ms에 재생하며 LLM을 호출하지 않습니다. 통계는 `engine.node_cache_stats()`로 확인합니다. |
...
| 21 | [`21_http2_transport.py`](21_http2_transport.py) | **OpenAI** | `SchemaProvider(..., prefer_libcurl=True)` — 옵트인 HTTP/2(libcurl) 전송과 기본 ConnPool(HTTP/1.1 keep-alive)을 비교합니다. libcurl이 빌드에 포함되면 5-way 병렬 버스트로 둘을 A/B 테스트하고, 해당 백엔드가 없는 wheel에서는 HTTP/1.1 smoke 호출 한 번을 수행한 뒤 HTTP/2 소스 빌드 방법을 안내합니다. |
