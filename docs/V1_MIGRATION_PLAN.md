...
| Durable child Program lifecycle (Issue #4) | **Implemented with conservative Level 1 authority.** | `start_child`, durable publication/dispatch/completion, recovery, timeout, duplicate-dispatch, and lineage tests exist. Child reservations attenuate `max_child_depth` to zero; recursive Levels 2–4 are not enabled. |
...
| Recursive child authority (Issue #12) | **Decision implemented conservatively.** | The runtime defaults to root-created leaf children: child depth is attenuated to zero and child creation is fail-closed without a future explicit grant. Levels 2–4 still require a separately admitted policy contract. |
