...
- **`update_state` は `ChannelWrite` の辞書またはリストを受け入れます** —
  ...
  リスト フォーム内の重複チャネルは、最後に書き込んだ方が優先されます。のために
  APPEND リデューサでのチャネルごとのマルチ書き込み (例: 2 つのチャネルの追加)
  メッセージを 1 回の呼び出しで)、値を値リストにバンドルします。
  `{"messages": [m1, m2]}`。他のタイプは代わりに `TypeError` を発生させます
  サイレントに no-op を実行します (項目 #5 によって閉じられた v0.3.2 より前のトラップ)。
