...
オフラインの `the_beast.cpp` は evolution.h の `evolve()` を実行しますが、そのパスは
`evaluate()` は **ゲートのみ**: ミュータントをコンパイル/検証し、それを呼び出します
「コスト 0」 — ハーネスを *実行* したり、その出力にスコアを付けたりすることはありません。そこでフィットネスというのは、
平らで何も登らない（最も優れた個体がシードに残る）。

[`the_beast_evolve.cpp`](the_beast_evolve.cpp) は欠落しているレイヤーを提供します。
**ハーネスを実行して実際の出力をスコアリングする実際のフィットネス**、
そしてミームループを駆動します。
