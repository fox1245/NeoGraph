...
実際の LLM 自動ロード `.env` を cwd (または任意の) からヒットする例
親) cppdotenv 経由。認識される 2 つのキーは次のとおりです。
...
| [`01_react_agent.cpp`](01_react_agent.cpp) |実際の LLM + 計算ツールを使用した ReAct ループ。 **`OPENAI_API_KEY` が必要です。** |
...
| 56 | [`56_history_compaction.cpp`](56_history_compaction.cpp) |オフライン (オプションの OpenAI) |境界付きメッセージ ウィンドウ - 履歴が予算を超えると、削除されたプレフィックスが LLM によって作成された概要に置き換えられます。デフォルトではモックプロバイダー。 |

### 本物の LLM — プロバイダー、ツール、ReAct

| # |ファイル |セットアップ |それが示すもの |
|---|------|-------|---------------|
| 01 | [`01_react_agent.cpp`](01_react_agent.cpp) |オープンAI | ReAct ループ: `llm_call` ↔ `tool_dispatch` (条件付き `has_tool_calls`)。計算ツール。 |
| 12 | [`12_rag_agent.cpp`](12_rag_agent.cpp) |オープンAI |実際の `text-embedding-3-small` 埋め込み + メモリ内コサイン検索を備えた RAG。 |
| 13 | [`13_openai_responses.cpp`](13_openai_responses.cpp) |オープンAI |同じ ReAct ループですが、`SchemaProvider("openai_responses")` 経由で `/v1/responses` に接続されています。構成を 1 つ反転するだけで、プロバイダーのサブクラスはありません。 |
| 33 | [`33_openai_responses_ws.cpp`](33_openai_responses_ws.cpp) |オープンAI | WebSocket 経由の応答 API — `wss://api.openai.com/v1/responses`。マルチツールのエージェント ループでの遅延が最大 40% 削減されます。 |
| 34 | [`34_openai_responses_ws_tools.cpp`](34_openai_responses_ws_tools.cpp) |オープンAI |すべての Responses-API 組み込みツール (web_search、image_generation、file_search、tool_search、skills、shell) を紹介します。ワイヤレベル (SchemaProvider なし)。 |
| 29 | [`29_responses_envelope.cpp`](29_responses_envelope.cpp) |オープンAI |デバッグ支援: 1 つのツール呼び出しリクエストの生の `/v1/responses` JSON エンベロープをダンプします。 SchemaProvider を意図的にバイパスします。 |
| 30 | [`30_reasoning_effort.cpp`](30_reasoning_effort.cpp) |オープンAI | 1 つのプロンプトで `reasoning_effort` を `{minimal, low, medium, high}` にスイープします。レイテンシー / 隠された CoT トークン / 応答品質のトレードオフを参照してください。 |
...
| 26 | [`26_postgres_react_hitl/`](26_postgres_react_hitl/) | OpenAI WS + Postgres + Crawl4AI |プロセス不連続ディープリサーチ HITL — PG でバックアップされたチェックポイントは、レポートと再開の間の `exit` まで存続します。 Docker-Compose 駆動。 |
...
| # |ファイル |セットアップ |それが示すもの |
|---|------|-------|---------------|
| 03 | [`03_mcp_agent.cpp`](03_mcp_agent.cpp) | OpenAI + MCP HTTP サーバー |ストリーミング可能な http MCP サーバーからツールを検出し、ReAct ループを駆動します。 |
| 22 | [`22_mcp_stdio.cpp`](22_mcp_stdio.cpp) | OpenAI + Python stdio スクリプト | 03 と同じですが、MCP サーバーは stdin/stdout 上の子サブプロセスであり、ネットワーク スタックはありません。 |
| 23 | [`23_mcp_multi.cpp`](23_mcp_multi.cpp) | OpenAI + 2 サーバー | 1 つのエージェント、2 つの MCP サーバー (HTTP + stdio)、ツールが 1 つのリストに統合され、LLM は両方を透過的に選択します。 |
...
| 20 | [`20_mcp_hitl.cpp`](20_mcp_hitl.cpp) | OpenAI + MCP HTTP サーバー | `interrupt_before` 任意の MCP ツール呼び出し - オペレーターは保留中のツール名 + 引数を確認し、承認し、再開します。 |
| 24 | [`24_mcp_feedback.cpp`](24_mcp_feedback.cpp) | OpenAI + MCP HTTP サーバー |オペレーターはエージェントの回答草案を読み、フィードバックを入力します。 2 回目の実行では、そのフィードバックが新しい会話のコンテキストとして組み込まれます。 |
...
| 40 | [`40_react_async_streaming.cpp`](40_react_async_streaming.cpp) |オープンAI |外側の `asio::io_context` + `co_spawn` + `co_await engine->run_stream_async(...)` は ReAct ループを駆動し、LLM ノードのトークンは `SchemaProvider("openai_responses")` に対して `co_await provider->complete_stream_async(...)` 経由で stdout にストリーミングされます。 **PR-#10 以前にセグメンテーション違反があった形状とまったく同じ** — 修正後は正常に実行されます。ツールの往復と最終的な回答は最大 4 秒で完了します。 |
...
| # |ファイル |セットアップ |それが示すもの |
|---|------|-------|---------------|
| 25 | [`25_deep_research.cpp`](25_deep_research.cpp) | Anthropic + Crawl4AI Docker | `langchain-ai/open_deep_research` の C++ ポート。スーパーバイザーは計画を立て、並行するサブ研究者 (それぞれ独自の ReAct ループ) を展開し、マークダウン レポートを統合します。 |
| 28 | [`28_corrective_rag.cpp`](28_corrective_rag.cpp) |オープンAI | CRAG (Yan et al. 2024)。関連性に応じて、取得→グレード→リファイン(KB)/リファイン+Web/Webのみにルーティングします。 `/v1/responses` 内蔵ツールによる Web 検索。 |
...
| 35 | [`35_re_agent.cpp`](35_re_agent.cpp) | OpenAI + ギドラ + ギドラ-mcp |リバース エンジニアリング エージェント — Ghidra を介して、ストリップされたバイナリから関数名と概要を復元します。エンドツーエンドで検証済み (matched_score 0.92、6-fn crashme)。 [`fox1245/re-agent`](https://github.com/fox1245/re-agent) の完全なパイプライン。 |
...
3. **非 OpenAI シェイプの場合は `SchemaProvider`** (13、15、16、17、33):
   1 つの JSON スキーマはワイヤーの形状 (Anthropic、Gemini、
   OpenAI Responses API、生の WebSocket)、同じ `Agent` /
   `llm_call` ノードはサブクラス化せずに別のエンドポイントにヒットします。
...
プロバイダーであり、キーが存在する場合にのみ OpenAI にタッチします。
