...
json structured_document(std::string_view mode) {
    const auto    call = [] { return json{{"op", "call_core"}}; };
    json          root;
    std::uint64_t operations  = 2;
    std::uint64_t concurrency = 1;
    if (mode == "sequence") {
        root = json{{"op", "sequence"}, {"children", json::array({call(), call()})}};
    } else if (mode == "parallel" || mode == "race") {
        root        = json{{"op", mode}, {"branches", json::array({call(), call()})}};
        concurrency = 2;
    } else if (mode == "quorum") {
        root =
            json{{"op", "quorum"}, {"branches", json::array({call(), call()})}, {"min_success", 2}};
        concurrency = 2;
    } else if (mode == "await") {
        root       = json{{"op", "await"}, {"body", call()}, {"timeout_ms", 1000}};
        operations = 1;
    } else {
        throw std::invalid_argument("unknown structured join mode: " + std::string(mode));
    }
    root["name"]       = "quickjs-bench-core";
    root["definition"] = core_definition();
    return {{"program_schema_version", 1},
            {"input_contract", {{"schema_version", 1}, {"schema", json::object()}}},
            {"output_contract", {{"schema_version", 1}, {"schema", json::object()}}},
            {"root", std::move(root)},
            {"declared_budget_requirements", budget_requirements(operations, concurrency)}};
}
...
void benchmark_structured_join(std::string_view case_id,
    ...
    const auto     version = fixture.admit(
        ProgramSource::from_cpp_builder("bench:structured-join", 1, structured_document(mode)));
    require_completed(
...
void print_replay_placeholder(std::size_t replay_count) {
    json output{{"schema_version", 1},
                {"case", "replay_growth"},
                {"status", "unavailable_until_q4"},
                {"replay_command_count", replay_count},
                {"reason", "deterministic recorded-command replay is not implemented at Q2"},
                {"build_type", build_type()}};
    std::cout << output.dump() << '\n';
}
...
int main(int argc, char** argv) {
    try {
        ...
        if (*case_id == "define_lowering") {
            ...
            print_replay_placeholder(*replay_count);
            return 3;
        } else {
