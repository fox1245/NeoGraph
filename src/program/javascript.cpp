...
void reject_dynamic_module_syntax(std::string_view source) {
    for (const std::string_view token : {"import", "require"}) {
        std::size_t search = 0;
        while (const auto found = find_javascript_token(source, token, search)) {
            auto cursor = *found + token.size();
            while (cursor < source.size() &&
                   std::isspace(static_cast<unsigned char>(source[cursor])) != 0)
                ++cursor;
            if (cursor < source.size() && source[cursor] == '(') {
                throw JavaScriptCompileError(
                    "P_JS_IMPORT_DYNAMIC", "Dynamic JavaScript module loading is forbidden",
                    json{{"engine", "quickjs"}, {"facility", "import"}},
                    source_span_at(source, *found, token.size()));
            }
...
void reject_unsealed_module_syntax(std::string_view source) {
    // A source without a verified closure retains the pre-existing public
    // error contract for every import form, including import(...).
...
class SealedModuleResolver final {
public:
    explicit SealedModuleResolver(const std::vector<SealedJavaScriptModule>& modules)
        : modules_(&modules) {}

    const SealedJavaScriptModule* find(std::string_view source_id) const noexcept {
        const auto found = std::find_if(
            modules_->begin(), modules_->end(), [&](const SealedJavaScriptModule& module) {
                return module.source_id == source_id;
            });
...
private:
    const std::vector<SealedJavaScriptModule>* modules_;
};

void reject_module_request(JSContext*       context,
                           std::string      code,
                           std::string      message,
                           std::string_view specifier,
                           bool             receipt_bound_source_available) {
    record_failure(
        static_cast<DefinitionCapture*>(JS_GetContextOpaque(context)), std::move(code), message,
        json{{"engine", "quickjs"},
             {"facility", "import"},
             {"specifier", std::string(specifier)},
             {"receipt_bound_source_available", receipt_bound_source_available}});
    JS_ThrowReferenceError(context, "%s", message.c_str());
}

char* sealed_module_normalize(JSContext* context,
                              const char*,
                              const char* module_name,
                              void*       opaque) {
    const auto specifier = module_name ? std::string_view(module_name) : std::string_view{};
    const auto* resolver = static_cast<const SealedModuleResolver*>(opaque);
    if (!resolver || !resolver->find(specifier)) {
        reject_module_request(context, "P_JS_IMPORT_UNAVAILABLE",
                              "JavaScript import is not bound to an exact sealed module receipt",
                              specifier, resolver && !resolver->empty());
        return nullptr;
    }
    return js_strdup(context, module_name);
}

JSModuleDef* sealed_module_loader(JSContext* context, const char* module_name, void* opaque) {
    const auto specifier = module_name ? std::string_view(module_name) : std::string_view{};
    const auto* resolver = static_cast<const SealedModuleResolver*>(opaque);
    const auto* module   = resolver ? resolver->find(specifier) : nullptr;
    if (!module) {
        reject_module_request(context, "P_JS_IMPORT_UNAVAILABLE",
                              "JavaScript import is not bound to an exact sealed module receipt",
                              specifier, resolver && !resolver->empty());
    ...
    JSValue compiled = JS_Eval(context, module->source_text.data(), module->source_text.size(),
                               module_name, JS_EVAL_TYPE_MODULE | JS_EVAL_FLAG_COMPILE_ONLY);
    if (JS_IsException(compiled)) {
        record_failure(
            static_cast<DefinitionCapture*>(JS_GetContextOpaque(context)), "P_JS_IMPORT_INVALID",
            "Sealed JavaScript module failed to compile",
            json{{"engine", "quickjs"},
                 {"facility", "import"},
                 {"specifier", module->source_id},
                 {"content_identity", module->content_identity}});
        return nullptr;
    }
    if (JS_VALUE_GET_TAG(compiled) != JS_TAG_MODULE) {
        JS_FreeValue(context, compiled);
        reject_module_request(context, "P_JS_IMPORT_INVALID",
                              "Sealed JavaScript module did not compile as an ES module",
                              specifier, true);
        return nullptr;
    }
    auto* result = static_cast<JSModuleDef*>(JS_VALUE_GET_PTR(compiled));
    JS_FreeValue(context, compiled);
    return result;
}
...
void install_sealed_module_resolver(QuickJsScope&             scope,
                                    const SealedModuleResolver& resolver) {
    JS_SetModuleLoaderFunc(scope.runtime(), sealed_module_normalize, sealed_module_loader,
                           const_cast<SealedModuleResolver*>(&resolver));
}
...
JavaScriptSourceEvaluation evaluate_javascript_source(const ProgramSource&           source,
    ...
    if (source.sealed_modules().empty())
        reject_unsealed_module_syntax(script);
    else
        reject_dynamic_module_syntax(script);

    const auto        started_at = std::chrono::steady_clock::now();
    SealedModuleResolver resolver(source.sealed_modules());
    QuickJsScope      scope(limits);
    install_sealed_module_resolver(scope, resolver);
    DefinitionCapture capture;
...
struct JavaScriptGenerator::Impl {
    ...
    explicit Impl(const JavaScriptCompileLimits&                       limits,
        ...
        : configured_limits(limits),
          source_envelope(std::move(source)),
          source_text(source_envelope.document().at("source").get<std::string>()),
          resolver(source_envelope.sealed_modules()),
    ...
    ProgramSource           source_envelope;
    std::string             source_text;
    SealedModuleResolver    resolver;
    QuickJsScope            scope;
    DefinitionCapture       capture;
    InterruptBudget         budget;
...
std::optional<JavaScriptGenerator> JavaScriptGenerator::open(
    ...
    auto impl = std::make_unique<Impl>(limits, source, std::move(cancellation_requested), deadline);
    auto* context = impl->scope.context();
